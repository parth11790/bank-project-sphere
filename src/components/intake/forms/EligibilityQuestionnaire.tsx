...
const EligibilityQuestionnaire: React.FC<FormComponentProps> = ({ formData, updateFormData }) => {
  ...
  const onSubmit = (values: z.infer<typeof eligibilitySchema>) => {
    // Convert optional principal_status properties to required for IntakeFormData
    ...
    updateFormData({
      ...values,
      principal_status,
      ineligible_business_types: values.ineligible_business_types || [],
      pre_screening_status: values.pre_screening_status,
      eligibility_notes: values.eligibility_notes || '',
    });
  };

  React.useEffect(() => {
    const subscription = form.watch((value) => {
      if (value.principal_status) {
        // Convert optional principal_status properties to required for IntakeFormData
        const principal_status = {
